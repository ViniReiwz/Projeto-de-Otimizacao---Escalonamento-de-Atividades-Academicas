...
# matricula[x] = vetor/lista de provas referentes ao aluno x
...
# ==================================================================
# Matriz das penalidades/punições baseada nas distâncias entre provas
# ==================================================================

# O cálculo das penalidades foi feito seguindo um padrão arbitrário
# decrescente (ou seja, quanto mais próximas, maior a punição)
    ...
    for j in E:
        if i < j:
            for t1 in T:
                for t2 in T:
                    if(t1 != t2):
                        y[i, j, t1, t2] = model.addVar(vtype=GRB.BINARY, name=f"y_{i}_{j}_{t1}_{t2}")
    ...
    for j in E:
        if i < j:
            for t1 in T:
                for t2 in T:
                    if(t1 != t2):
                        model.addConstr(y[i, j, t1, t2] <= x[i, t1])
                        model.addConstr(y[i, j, t1, t2] <= x[j, t2])
                        model.addConstr(y[i, j, t1, t2] >= x[i, t1] + x[j, t2] - 1)
...
model.setParam("TimeLimit", 300)  # limite proposto de 300s para execução do modelo
...
for i in E:
    for t in T:
        if x[i, t].X > 0.5:
            print(f"{num_provas[i]} -> {num_periodos[t]}")
